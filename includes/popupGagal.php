...
<body class="bg-gray-200 min-h-screen flex items-center justify-center p-4 md:p-10">
    <div class="fixed top-0 left-0 w-full h-full bg-black/50 flex justify-center items-center z-[9999] p-4">
        ...
        <div class="bg-white rounded-[20px] py-[35px] px-[30px] md:py-[50px] md:px-[50px] flex flex-col items-center gap-[20px] md:gap-[30px] w-full max-w-[320px] md:max-w-[400px] lg:max-w-[450px] shadow-2xl transition-all">
            ...
            <?= $popup_type == 'error' ? 'bg-red-500/10' : 'bg-primary/10' ?> 
            rounded-full flex justify-center items-center p-3">
                <div class="w-full h-full rounded-full 
                <?= $popup_type == 'error' ? 'bg-red-500' : 'bg-primary' ?> 
                flex justify-center items-center shadow-lg">
                    <img src="assets/img/<?= $popup_type == 'error' ? 'silang.png' : 'centang2.png' ?>" 
                    class="w-[28px] md:w-[40px] h-auto">
                </div>
            </div>

            <div class="flex flex-col text-center justify-center items-center gap-[10px] md:gap-[15px]">
                <h2 class="font-bold text-xl md:text-3xl text-red-500">Login Gagal!</h2>
                <p class="text-sm md:text-lg text-red-500 leading-relaxed px-2">
                    <?php echo $pesan_error1?>
                </p>
            </div>

            <div class="w-full pt-2 md:pt-4">
                <a href="index.php" class="w-full h-[50px] md:h-[65px] bg-red-500 rounded-[15px] text-[15px] md:text-[18px] text-white font-bold tracking-widest hover:opacity-90 active:scale-[0.98] transition-all shadow-lg shadow-primary/10 uppercase flex items-center justify-center gap-2">
                    Kembali ke Login
                </a>
            </div>
        </div>
