        ...
        <div class="w-full max-w-5xl mx-auto flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-3">
                ...
                <?php while($ps = $qRiwayat->fetch_assoc()):
                    ...
                    $badge = match($ps['status_pesanan']) {
                        'selesai'       => "<span class='px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full'>Siap Diambil</span>",
                        'diambil'       => "<span class='px-3 py-1 bg-teal-100 text-teal-700 text-xs font-bold rounded-full'>Diambil</span>",
                        'tidak_diambil' => "<span class='px-3 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded-full'>Tidak Diambil</span>",
                        'dibatalkan'    => "<span class='px-3 py-1 bg-red-100 text-red-600 text-xs font-bold rounded-full'>Dibatalkan</span>",
                        default         => "<span class='px-3 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded-full'>" . htmlspecialchars($ps['status_pesanan']) . "</span>",
                    };
                    
                    $poin_dig    = (int)($ps['poin_digunakan'] ?? 0);
                    $kode_unik   = (int)($ps['kode_unik']      ?? 0);
                    $harga_asli  = (int)($ps['total_harga']    ?? 0);
                    // Rekonstruksi harga sebelum kode unik (karena kode_unik sudah di-set 0 setelah selesai/diambil)
                    // Namun poin_digunakan masih tersimpan di pesanan
                    // Harga bruto = harga yang diterima penjual = total_harga (sudah dikurangi poin & kode unik)
                    // Kita tampilkan breakdown:
                    $harga_asli_bruto = $harga_asli + $poin_dig; // sebelum diskon poin
                    $info_diskon = '';
                    if ($poin_dig > 0) {
                        $info_diskon .= "<span class='text-[11px] text-text-3'>Diskon poin: <span class='text-red-500 font-semibold'>-Rp " . number_format($poin_dig, 0, ',', '.') . "</span></span>";
                    }
                ?>
                <div class="bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 flex flex-col gap-3 hover:-translate-y-0.5 hover:shadow-md transition-all">
                    ...
                    <div class="flex justify-between items-center border-t border-gray-100 pt-3">
                        ...
                        <div class="flex flex-col items-end gap-0.5">
                            <?php if($poin_dig > 0): ?>
                            <span class="text-[11px] text-text-3">Harga dasar: <span class="font-semibold">Rp <?= number_format($harga_asli_bruto, 0, ',', '.') ?></span></span>
                            <span class="text-[11px] text-red-500">Diskon poin siswa: -Rp <?= number_format($poin_dig, 0, ',', '.') ?></span>
                            <?php endif; ?>
                            <p class="text-base font-bold text-text-1">Rp <?= number_format($harga_asli, 0, ',', '.') ?></p>
                        </div>
                    </div>
                </div>
                <?php endwhile; ?>
